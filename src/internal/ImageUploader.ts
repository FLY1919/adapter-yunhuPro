...
export class ImageUploader extends BaseUploader
{
    ...
    {
        this.bot.logInfo(`开始处理图片上传，传入参数类型: ${typeof image}`);

        if (Buffer.isBuffer(image))
        {
            this.bot.logInfo('传入参数是 Buffer 类型');
        } else if (typeof image === 'string')
        {
            this.bot.logInfo(`传入参数是字符串类型，内容: ${image.substring(0, 100)}...`);

            if (image.startsWith('data:image/'))
            {
                this.bot.logInfo('检测到 base64 编码的图片数据');
            } else if (image.startsWith('http://') || image.startsWith('https://'))
            {
                this.bot.logInfo('检测到 HTTP/HTTPS URL');
            } else if (image.startsWith('file://'))
            {
                this.bot.logInfo('检测到 file:// URL');
            } else if (this.isFilePath(image))
            {
                this.bot.logInfo('检测到文件路径');
            }
        } else
        {
            this.bot.logInfo(`传入参数是其他类型: ${image?.constructor?.name || '未知类型'}`);
        }
        ...
        this.bot.logInfo(`最终文件名: ${finalFileName}`);
    ...
    {
        this.bot.logInfo(`开始解析图片资源，类型: ${typeof image}`);

        // 如果是Buffer直接返回
        if (Buffer.isBuffer(image))
        {
            this.bot.logInfo('资源是 Buffer 类型，开始检测MIME类型');
            // 验证Buffer内容是否为有效图片
            const mimeType = await this.detectMimeType(image);
            this.bot.logInfo(`Buffer 检测到的MIME类型: ${mimeType}`);

            if (!mimeType.startsWith('image/'))
            {
                this.bot.loggerError('提供的Buffer不是有效的图片数据');
        ...
        {
            this.bot.logInfo(`资源是字符串类型: ${image.substring(0, 50)}...`);
            ...
            if (image.startsWith('data:image/'))
            {
                ...
                if (matches && matches.length === 3)
                {
                    ...
                    this.bot.logInfo(`Base64 数据解码成功，长度: ${buffer.length} 字节`);

                    // 验证base64数据是否为有效图片
                    const mimeType = await this.detectMimeType(buffer);
                    this.bot.logInfo(`Base64 数据检测到的MIME类型: ${mimeType}`);

                    if (!mimeType.startsWith('image/'))
                    {
                        this.bot.loggerError('提供的base64数据不是有效的图片');
            ...
            {
                this.bot.logInfo('检测到 HTTP/HTTPS URL，开始下载图片');
                try
                {
                    // 使用HTTP客户端下载图片
                    ...
                    if (!mimeType.startsWith('image/'))
                    {
                        this.bot.loggerError('从URL下载的内容不是有效的图片');
                    ...
                    {
                        this.bot.logInfo('URL解析失败，使用默认文件名');
                    }
                ...
                {
                    this.bot.loggerError(`无法下载或验证URL图片: ${error.message}`);
            ...
            {
                this.bot.logInfo('检测到文件路径，开始读取文件');
                // 处理file://协议
                let filePath = image;
                if (image.startsWith('file://'))
                {
                    ...
                    {
                        this.bot.logInfo('file:// URL解析失败，回退到简单处理');
                ...
                this.bot.logInfo(`规范化后的文件路径: ${normalizedPath}`);

                // 检查文件是否存在
                if (!fs.existsSync(normalizedPath))
                {
                    this.bot.loggerError(`文件不存在: ${normalizedPath}`);
                    throw new Error(`文件不存在: ${normalizedPath}`);
                }

                // 读取文件
                const buffer = await fs.promises.readFile(normalizedPath);
                this.bot.logInfo(`文件读取成功，长度: ${buffer.length} 字节`);

                // 验证文件内容是否为有效图片
                const mimeType = await this.detectMimeType(buffer);
                this.bot.logInfo(`文件内容检测到的MIME类型: ${mimeType}`);

                if (!mimeType.startsWith('image/'))
                {
                    this.bot.loggerError(`文件不是有效的图片: ${normalizedPath}`);
            ...
            this.bot.logInfo('无法识别的字符串格式，尝试使用默认解析方法');
        }

        this.bot.logInfo('使用默认的 resolveResource 方法解析资源');
        ...
        this.bot.logInfo(`resolveResource 检测到的MIME类型: ${mimeType}`);

        if (!mimeType.startsWith('image/'))
        {
            this.bot.loggerError('解析的资源不是有效的图片');
    ...
    {
        this.bot.logInfo('开始检测 Buffer 的 MIME 类型');

        // 首先检查文件签名（magic numbers）
        const detectedType = this.detectMimeTypeBySignature(buffer);
        if (detectedType)
        {
            this.bot.logInfo(`通过文件签名检测到的 MIME 类型: ${detectedType}`);
            return detectedType;
        }

        // 如果文件签名检测失败，使用 file-type 库
        this.bot.logInfo('文件签名检测失败，尝试使用 file-type 库');
        const fileType = await fileTypeFromBuffer(buffer);

        if (fileType)
        {
            this.bot.logInfo(`file-type 库检测到的 MIME 类型: ${fileType.mime}`);
            return fileType.mime;
        }

        this.bot.logInfo('file-type 库也无法检测到 MIME 类型');

        // 如果 file-type 也检测失败，尝试通过常见文件扩展名推断
        const extensionBasedType = this.guessMimeTypeByContent(buffer);
        if (extensionBasedType)
        {
            this.bot.logInfo(`通过内容推断的 MIME 类型: ${extensionBasedType}`);
            return extensionBasedType;
        }

        this.bot.logInfo('所有检测方法都失败，返回默认类型');
    ...
    {
        this.bot.logInfo(`检查字符串是否为文件路径: ${str}`);

        // 检查是否包含路径分隔符
        if (str.includes(path.sep))
        {
            this.bot.logInfo('字符串包含路径分隔符，可能是文件路径');
            return true;
        }
        ...
        if (/^[a-zA-Z]:[\\/]/.test(str))
        {
            this.bot.logInfo('字符串是 Windows 风格路径');
            return true;
        }
        ...
        {
            this.bot.logInfo('字符串是 Unix 风格绝对路径');
            return true;
        }
        ...
        if (str.startsWith('./') || str.startsWith('../'))
        {
            this.bot.logInfo('字符串是相对路径');
            return true;
        }

        this.bot.logInfo('字符串不是文件路径');
        return false;
    }
}